...
namespace Glaxion.Music
{
    public class MusicPlayer
    {
        private void Construction()
        {
            ...
            PlayEvent += new PlayEventHandler(On_Play);
            PauseEvent += new PauseEventHandler(On_Pause);
            ResumeEvent += new ResumeEventHandler(On_Resume);
            NextEvent += new NextEventHandler(On_Next);
            PrevEvent += new PrevEventHandler(On_Prev);
            StopEvent += new StopEventHandler(On_Stop);
            TickEvent += new TickEventHandler(On_Tick);
            DirectoriesLoadedEvent += new DirectoriesLoadedEventHandler(On_DirectoriesLoaded);
            TrackChangeEvent += new TrackChangeEventHandler(On_TrackChange);
        ...
        public bool Play(int index)
        {
            ...
            if(playlist == null)
            {
                //tool.debug("no playlist assigned");
                PlaybackFailedEvent("Playlist is Missing", EventArgs.Empty);
                Stop();
            ...
            if (!File.Exists(file))
            {
                //tool.debug("Music Player: Cannot Find file: " + file);
            ...
            //return true;
            //musicApplication.tr;
            //should not be doing this from here

            // musicApplication.trackInfo_mainControl.ShowInfo(file);
            //string image = musicApplication.trackInfo_mainControl.trackInfo.image;
            //musicApplication.picturePanel1.Updatebackground(image);
